...
- 不需要提前准备背景音乐：需求访谈会确认是否使用 BGM；不需要时可以明确选择不加。
...
本仓库只提供口播视频的需求确认与定制决策层。实际工程创建、时间线、媒体处理、检查和 Studio 预览由 HyperFrames 基础 Skills 负责。
...
调用时可以先只告诉 Codex 素材位置和目标。Skill 会继续确认：制作范围、发布平台、画幅与 fps、视觉风格、动效强度、字幕、可用素材、人物与安全区、原声/BGM/SFX，以及先看方案、先做开头 V1、全片 Studio 预览还是最终需要 MP4。

第一次回复只询问需求，不读取素材或修改工程。确认《需求确认单》后才开始制作；Studio 审核完成后，还需要单独批准才能渲染最终 MP4。
